...
package moe.lz233.fuckweread.utils.ktx
...
fun Class<*>.hookMethod(method: String?, vararg args: Any?) = try {
    LogUtil._d(tag = "HOOKWEREAD", obj = "Hook ${name}.$method Successful")
...
fun String.hookMethod(
    method: String?,
    vararg args: Any?,
    classLoader: ClassLoader = ModuleContext.classLoader
) = try {
...
fun String.findClass(classLoader: ClassLoader? = ModuleContext.classLoader): Class<*> =
    findClass(this, classLoader)

fun String.findClassOrNull(classLoader: ClassLoader? = ModuleContext.classLoader): Class<*>? =
